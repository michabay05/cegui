...
void generatePawns(MoveList &moveList, const Board &board) {
  ...
  while (bitboardCopy) {
    ...
    if ((board.state.side == Color::WHITE ? target >= (int)Sq::a8
                                          : target <= (int)Sq::h1) &&
      ...
      if ((source >= promotionStart) && (source <= promotionStart + 7)) {
        moveList.add(encode(
            source, target, piece,
            (board.state.side == Color::WHITE ? (int)Piece::Q : (int)Piece::q),
            0, 0, 0, 0));
        moveList.add(encode(
            source, target, piece,
            (board.state.side == Color::WHITE ? (int)Piece::R : (int)Piece::r),
            0, 0, 0, 0));
        moveList.add(encode(
            source, target, piece,
            (board.state.side == Color::WHITE ? (int)Piece::B : (int)Piece::b),
            0, 0, 0, 0));
        moveList.add(encode(
            source, target, piece,
            (board.state.side == Color::WHITE ? (int)Piece::N : (int)Piece::n),
            0, 0, 0, 0));
      } else {
        ...
        if ((source >= doublePushStart && source <= doublePushStart + 7) &&
          ...
          moveList.add(encode(source, target + direction, piece, (int)Piece::E,
                              0, 1, 0, 0));
      }
    }
    // Capture moves
    ...
    if (board.state.enpassant != Sq::noSq) {
      uint64_t enpassCapture =
          Attack::pawnAttacks[(int)board.state.side][source] &
          (1ULL << (int)board.state.enpassant);
      if (enpassCapture) {
        int enpassTarget = Bitboard::lsbIndex(enpassCapture);
        moveList.add(
            encode(source, enpassTarget, piece, (int)Piece::E, 1, 0, 1, 0));
      }
    }
    // Remove bits
...
void generateKnights(MoveList &moveList, const Board &board) {
  int source, target,
      piece = board.state.side == Color::WHITE ? (int)Piece::N : (int)Piece::n;
  ...
  while (bitboardCopy) {
    ...
    attackCopy = Attack::knightAttacks[source] &
                 (board.state.side == Color::WHITE
                      ? ~board.pos.units[(int)Color::WHITE]
                      : ~board.pos.units[(int)Color::BLACK]);
    while (attackCopy) {
      target = Bitboard::lsbIndex(attackCopy);
      if (getBit(board.pos.units[board.state.side == Color::WHITE
                                     ? (int)Color::BLACK
                                     : (int)Color::WHITE],
                 target))
        moveList.add(encode(source, target, piece, (int)Piece::E, 1, 0, 0, 0));
...
void generateBishops(MoveList &moveList, const Board &board) {
  int source, target,
      piece = board.state.side == Color::WHITE ? (int)Piece::B : (int)Piece::b;
  uint64_t bitboardCopy = board.pos.pieces[piece], attackCopy;
  while (bitboardCopy) {
    source = Bitboard::lsbIndex(bitboardCopy);

    attackCopy =
        Magics::getBishopAttack(source, board.pos.units[(int)Color::BOTH]) &
        (board.state.side == Color::WHITE
             ? ~board.pos.units[(int)Color::WHITE]
             : ~board.pos.units[(int)Color::BLACK]);
    while (attackCopy) {
...
void generateRooks(MoveList &moveList, const Board &board) {
  int source, target,
      piece = board.state.side == Color::WHITE ? (int)Piece::R : (int)Piece::r;
  uint64_t bitboardCopy = board.pos.pieces[piece], attackCopy;
  while (bitboardCopy) {
    source = Bitboard::lsbIndex(bitboardCopy);

    attackCopy =
        Magics::getRookAttack(source, board.pos.units[(int)Color::BOTH]) &
        (board.state.side == Color::WHITE
             ? ~board.pos.units[(int)Color::WHITE]
             : ~board.pos.units[(int)Color::BLACK]);
    while (attackCopy) {
...
void generateQueens(MoveList &moveList, const Board &board) {
  int source, target,
      piece = board.state.side == Color::WHITE ? (int)Piece::Q : (int)Piece::q;
  uint64_t bitboardCopy = board.pos.pieces[piece], attackCopy;
  while (bitboardCopy) {
    source = Bitboard::lsbIndex(bitboardCopy);

    attackCopy =
        Magics::getQueenAttack(source, board.pos.units[(int)Color::BOTH]) &
        (board.state.side == Color::WHITE
             ? ~board.pos.units[(int)Color::WHITE]
             : ~board.pos.units[(int)Color::BLACK]);
    while (attackCopy) {
...
void generateKings(MoveList &moveList, const Board &board) {
  ...
  */
  int source, target,
      piece = board.state.side == Color::WHITE ? (int)Piece::K : (int)Piece::k;
  ...
  if (board.state.side == Color::WHITE) // WHITE
  {
    ...
    if (getBit(board.state.castling, (int)CastlingRights::wk)) {
      ...
      if (!getBit(board.pos.units[(int)Color::BOTH], (int)Sq::f1) &&
        ...
        if (!board.isSquareAttacked((int)Sq::e1) &&
            !board.isSquareAttacked((int)Sq::f1))
          moveList.add(encode((int)Sq::e1, (int)Sq::g1, (int)Piece::K,
                              (int)Piece::E, 0, 0, 0, 1));
      }
    }
    // Queenside castling
    if (getBit(board.state.castling, (int)CastlingRights::wq)) {
      ...
      if (!getBit(board.pos.units[(int)Color::BOTH], (int)Sq::b1) &&
        ...
        if (!board.isSquareAttacked((int)Sq::d1) &&
            !board.isSquareAttacked((int)Sq::e1))
          moveList.add(encode((int)Sq::e1, (int)Sq::c1, (int)Piece::K,
                              (int)Piece::E, 0, 0, 0, 1));
      }
    }
  }
...
void genBlackCastling(MoveList &moveList, const Board &board) {
  ...
  if (getBit(board.state.castling, (int)CastlingRights::bk)) {
    ...
    if (!getBit(board.pos.units[(int)Color::BOTH], (int)Sq::f8) &&
      ...
      if (!board.isSquareAttacked((int)Sq::e8) &&
          !board.isSquareAttacked((int)Sq::f8))
        moveList.add(encode((int)Sq::e8, (int)Sq::g8, (int)Piece::k,
                            (int)Piece::E, 0, 0, 0, 1));
    }
  }
  // Queenside castling
  if (getBit(board.state.castling, (int)CastlingRights::bq)) {
    ...
    if (!getBit(board.pos.units[(int)Color::BOTH], (int)Sq::b8) &&
      ...
      if (!board.isSquareAttacked((int)Sq::d8) &&
          !board.isSquareAttacked((int)Sq::e8))
        moveList.add(encode((int)Sq::e8, (int)Sq::c8, (int)Piece::k,
                            (int)Piece::E, 0, 0, 0, 1));
    }
  }
}
...
bool make(Board *main, const int move, MoveType moveFlag) {
  if (moveFlag == MoveType::allMoves) {
    ...
    if (capture) {
      for (int bbPiece = (main->state.side == Color::WHITE ? (int)Piece::p
                                                           : (int)Piece::P);
           bbPiece <=
           (main->state.side == Color::WHITE ? (int)Piece::k : (int)Piece::K);
           bbPiece++) {
